...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (userExists) {
        res.status(400)
        throw new Error(userExists.email === email ? 'Email already in use' : 'Name already taken') // 400 Bad Request — client sent invalid data (duplicate email or name)
    }
    ...
    const user = await User.create({
        ...
        plan // Again checking data types
    })
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const user = await User.findOne({ $or: [{ email: alias }, { name: alias }] }) // from database but encrypted
