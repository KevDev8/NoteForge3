...
const API_URL = 'https://noteforge3.onrender.com/api' // make sure to change this later
...
if (registerForm) {
  registerForm.addEventListener("keydown", e => {
    if (e.key === "Enter") {
      e.preventDefault(); // To avoid submissions without a chosen plan
    }
  })
  ...
  registerForm.addEventListener('submit', async (e) => {
    ...
    const plan = e.submitter.name; // Extract the name of the button containing the plan ID
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/users`, {
        ...
        body: JSON.stringify({ name, email, password, plan })  // convert JS object to a JSON string
      })
...
if (loginForm) {
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      // Send a POST request to /api/users/login with alias and password
